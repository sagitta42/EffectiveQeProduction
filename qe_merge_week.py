...
def mergedst(folder):
	...
	ffiles = [f for f in ffiles if 'Week' in f]
	...
	fdf['Date'] = [pd.to_datetime(f.split('_Week')[0].replace('_', '-')) for f in ffiles]
...
#	for f in ffiles:
		print f
		df = pd.read_csv(folder + '/' + f) # new sep: comma
#		df['Dst'] = f.split('_QE')[0] # already present in new QE
        # date info already there
